...
const Header = ({whom}) => {
    ...
    const navLinks = [
        ...
        { to: "/dashboard/senioradmin", name: "Dashboard", for:"senioradmin" },
                            ...
                            <nav className="primary-menu navbar navbar-expand-lg">
                                <div id="header-nav" className="collapse navbar-collapse">
                                    <ul className="navbar-nav me-auto">
                                        { navLinks.filter(items => items.for == whom).map((link, index) => <li key={index + link.to}  className={pathname === link.to ? "active" : ""}><Link to={link.to}>{link.name}</Link></li>)}
                                    </ul>
                                </div>
                            </nav>
                        </div>
                        <div className="header-column justify-content-end">
                            <nav className="login-signup navbar navbar-expand">
                                <ul className="navbar-nav">
                                    <li className="dropdown language"> <a className="dropdown-toggle" data-bs-toggle="dropdown" href="#">En</a>
                                    ...
                                    <li className="dropdown notifications"><a className="dropdown-toggle"  data-bs-toggle="dropdown" href="#"><span className="text-5"><i className="far fa-bell"></i></span><span className="count">3</span></a>
                                        <ul className="dropdown-menu dropdown-menu-end dropdown-menu-lg-start">
                                    ...
                                    <li className="dropdown profile ms-2">
                                        <a className="px-0 dropdown-toggle" data-bs-toggle="dropdown" href="#"><img className="rounded-circle" src="/images/profile-thumb-sm.jpg" alt="" /></a>
                                        <ul className="dropdown-menu dropdown-menu-end dropdown-menu-lg-start">
